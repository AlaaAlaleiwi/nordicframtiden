...
public class SalaryController {
  ...
  public List<MonthRow> userMonths(@RequestParam Long userId, @RequestParam int year) {
    ...
    return byMonth.entrySet().stream()
        .map(e -> {
          ...
          BigDecimal cost = e.getValue().stream().map(ShiftLine::cost).reduce(BigDecimal.ZERO, BigDecimal::add);
  ...
  public List<DayRow> userMonthDays(@RequestParam Long userId,
    ...
    return byDay.entrySet().stream()
        .map(e -> {
          ...
          OffsetDateTime from = dayLines.stream().map(ShiftLine::startAt).min(Comparator.naturalOrder()).orElse(null);
          OffsetDateTime to = dayLines.stream().map(ShiftLine::endAt).max(Comparator.naturalOrder()).orElse(null);
  ...
  private ShiftLine toLine(ScheduleShift s) {
    ...
    if (p == null) throw new IllegalStateException("Shift pharmacy is null (salary expects pharmacy shifts only)");
